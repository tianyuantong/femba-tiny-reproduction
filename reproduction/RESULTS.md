...
旋转实验检验特征幅值重分布对静态 PTQ 的影响，并为后续 QAT 提供输入。
...
均未通过原逐元素门槛。FP64 仅用于[隔离诊断](rotation_numeric_results.json)。
2026-09-22 据此将旋转中间张量改为逐特征向量 RMS 检查，最终 logits 逐元素门槛不变；
固定旋转基并要求结构检查拒绝 H4-only 等负对照，详见[协议修订](PROTOCOL.md#2026-09-22旋转数值检查修订)。
结构正对照和 6 个负对照、训练／验证探针的 RMS 和 logits 检查均通过，
四个 Rot 版本已完成全量评估。Rot-W8A32 在精度容差内；Rot-W4A32 相对未旋转组
AUROC 低 2.2996 个百分点，两组 Rot-A8 仍全部预测正类。旧失败证据保留。
